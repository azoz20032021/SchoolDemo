                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                    <div className="relative">
                        ...
                        <input
                            ...
                            dir="ltr"
                            className="w-full pr-11 pl-4 py-3 rounded-xl border border-slate-200 text-center tracking-[0.25em] font-black text-slate-800 outline-none focus:ring-2 focus:ring-brand-500"
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-brand-700 text-white py-3 rounded-xl font-semibold shadow-lg shadow-brand-200 disabled:opacity-60 flex items-center justify-center gap-2"
                    >
                    ...
                    >
                        ...
                        {result.status === 'approved' && result.assigned_uid && (
                            <div className="mt-4 bg-white/70 rounded-xl p-4 text-center">
                                ...
                                {/*
                                  * The number travels in the router's own state
                                  * rather than the address bar. It ends up in
                                  * the field either way, but a query string is
                                  * kept in history, in bookmarks and in every
                                  * server log it passes, and this is the one
                                  * number that identifies a child.
                                  */}
                                <Link
                                    to="/login"
                                    state={{ uid: result.assigned_uid }}
                                    className="mt-3 inline-block bg-emerald-600 text-white text-xs font-black px-5 py-2.5 rounded-xl shadow-lg shadow-emerald-500/25"
                                >
                                    {t('تسجيل الدخول الآن')}
                                </Link>
                            </div>
