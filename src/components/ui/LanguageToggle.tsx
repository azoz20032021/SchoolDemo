...
export const LanguageToggle: React.FC<{ variant?: 'icon' | 'full' }> = ({ variant = 'icon' }) => {
    ...
    if (variant === 'full') {
        return (
            <button
                type="button"
                onClick={toggleLang}
                className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-brand-700 transition-colors"
            >
    ...
    return (
        <button
            ...
            title={target}
            aria-label={target}
            className="px-2.5 py-2 rounded-xl text-[11px] font-black text-slate-500 hover:text-brand-700 hover:bg-slate-50 transition-all flex items-center gap-1"
        >
            <Languages className="w-4 h-4" />
