...
const inputClass =
    'w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all bg-white';
            ...
            <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
                ...
                >
                    ...
                    <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl p-5 mb-6">
                        ...
                        <p className="text-3xl font-black text-brand-700 tracking-[0.2em]" dir="ltr">{trackingCode}</p>
                        <button
                            onClick={copyCode}
                            className="mt-3 inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-brand-700 transition-colors"
                        >
                    ...
                    <div className="space-y-2">
                        <Link
                            to="/register/status"
                            className="block w-full bg-brand-700 text-white py-3 rounded-xl font-semibold shadow-lg shadow-brand-200"
                        >
                            {t('متابعة حالة الطلب')}
                        </Link>
                        ...
                        <div key={s.title} className="flex-1">
                            <div
                                className={`h-1.5 rounded-full transition-colors ${
                                    i < step ? 'bg-emerald-500' : i === step ? 'bg-brand-700' : 'bg-slate-200'
                                }`}
                            />
                        </div>
                    ))}
                ...
                <form onSubmit={handleSubmit} className="space-y-3.5">
                                        ...
                                        <button
                                            ...
                                            className={`py-2.5 rounded-xl text-sm font-black border transition-colors ${
                                                form.applicant_role === key
                                                    ? 'bg-brand-700 text-white border-brand-700 shadow-lg shadow-brand-100'
                    ...
                    {step === 1 && (
                        <>
                            <div className="grid grid-cols-2 gap-3">
                                <Field
                                    label={isTeacher ? t('رقم الهاتف') : t('هاتف الطالب')}
                                    required
                                    hint={t('يبدأ بـ 07 ومكوّن من 11 رقماً')}
                                >
                                ...
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <Field
                                            ...
                                            required
                                            hint={t('بهذا الرقم يدخل ولي الأمر لمتابعتك، وهو كلمة مروره الأولى')}
                                        >
                    ...
                    <div className="flex items-center gap-3 pt-2">
                        ...
                        {step < STEPS.length - 1 ? (
                            <button
                                type="button"
                                onClick={goNext}
                                className="flex-1 bg-brand-700 text-white py-3 rounded-xl font-semibold shadow-lg shadow-brand-200 flex items-center justify-center gap-1"
                            >
                                {t('التالي')}
