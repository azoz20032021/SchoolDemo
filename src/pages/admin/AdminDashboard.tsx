...
import { Users, BookOpen, Plus, ChevronRight, Trash2, ArrowRight, UserX, Search, Pencil, UserPlus } from 'lucide-react';
...
export const AdminDashboard: React.FC = () => {
    ...
    /** The student whose parent is being given an account. */
    const [guardianFor, setGuardianFor] = useState<Record<string, any> | null>(null);
    ...
    /**
     * Give a parent their own sign-in. The name and phone already recorded
     * against the student are used, and a parent who already has an account on
     * that number simply gains another child rather than a second password.
     */
    const createGuardian = async (e: React.FormEvent) => {
        ...
        try {
            const res = await api.post<{ linked: boolean; uid: string; name: string }>(
                `/api/admin/students/${guardianFor.id}/guardian`
            );
            setGuardianFor(null);
            alert(
                res.linked
                    ? t('تم ربط الطالب بحساب ولي الأمر الموجود: {name} ({uid})', { name: res.name, uid: res.uid })
                    : t('تم إنشاء حساب ولي الأمر. يدخل برقم {uid} وكلمة المرور نفس الرقم، وسيغيّرها بأول دخول.', { uid: res.uid })
            );
        } catch (err) {
            alert(err instanceof ApiError ? err.message : t('تعذر إنشاء حساب ولي الأمر'));
        } finally {
    ...
    return (
        <div className="p-4 md:p-6 space-y-4 md:space-y-6">
            {!selectedClass ? (
                <>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                        ...
                        {canManageStudents && (
                            ...
                            >
                                ...
                                <div className="absolute -right-4 -bottom-4 w-20 h-20 bg-brand-500/10 rounded-full blur-2xl"></div>
                            </button>
                        )}
                    </div>

                    {/* UID Management Section — account administration, admin only. */}
                    {canManageStudents && (
                    <div className="bg-white p-6 rounded-3xl border border-brand-100 shadow-sm space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className="text-brand-700 text-xs font-bold px-3 py-1.5 bg-brand-50 rounded-full hover:bg-brand-100 transition-colors"
                            >
                            ...
                            <div className="space-y-6 pt-2 animate-in fade-in slide-in-from-top-4 duration-300">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-3">
                                        ...
                                        <button
                                            ...
                                            className="w-full bg-brand-700 text-white py-2.5 rounded-xl text-xs font-black shadow-lg shadow-brand-100 active:scale-95 transition-all"
                                        >
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between px-1">
                                        ...
                                        <button onClick={fetchData} className="text-[10px] text-brand-700 font-bold hover:underline">{t('تحديث القائمة')}</button>
                                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            ...
                            {canManageClasses && (
                                <button
                                    ...
                                    className="text-brand-700 text-sm font-medium flex items-center gap-1"
                                >
                        ...
                        {showAddClass && (
                            <form onSubmit={handleAddClass} className="bg-white p-4 rounded-2xl border border-emerald-100 shadow-sm space-y-3">
                                        ...
                                        {teachers.map(teacher => (
                                            <button
                                                ...
                                                className={`px-3 py-1.5 rounded-full text-[10px] font-bold transition-all border ${selectedTeacherIds.includes(teacher.id)
                                                    ? 'bg-brand-700 text-white border-brand-700 shadow-sm shadow-brand-100'
                                ...
                                <button type="submit" className="w-full bg-brand-700 text-white py-2 rounded-xl text-sm font-bold">
                                    {t('إضافة الآن')}
                                </button>
                            </form>
                        )}
                        ...
                        <div className="space-y-3">
                            {classes.map(c => (
                                <div key={c.id}
                                    ...
                                    className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between transition-all hover:border-brand-200 cursor-pointer"
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-slate-500 mt-0.5 font-sans">
                                                {t('المعلمون')}: {c.teacher_names && c.teacher_names.length > 0
                    ...
                    <div className="space-y-4">
                        ...
                        {showAddTeacher && (
                            <form onSubmit={handleAddTeacher} className="bg-white p-4 rounded-2xl border border-brand-100 shadow-sm space-y-3">
                                <input
                                    type="text"
                        ...
                        <div className="grid grid-cols-2 gap-3 pb-2">
                            {teachers.map(teacher => (
                                <div key={teacher.id}
                                    ...
                                    className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col items-center gap-2 cursor-pointer hover:border-brand-200 transition-all font-sans"
                                >
                        ...
                        {teachersNextCursor && (
                            <div className="text-center pb-4">
                                <button
                                    ...
                                    className="px-4 py-2 bg-brand-50 hover:bg-brand-100 text-brand-700 rounded-xl text-xs font-bold transition-all disabled:opacity-50"
                                >
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pr-10 px-4 py-2.5 rounded-xl border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent transition-all bg-white"
                                placeholder={t('ابحث باسم الطالب أو رقمه التعريفي')}
                            />
                        </div>

                        {showAddStudent && (
                            <form onSubmit={handleAddStudent} className="bg-white p-4 rounded-2xl border border-brand-100 shadow-sm space-y-3">
                                <input
                                    type="text"
                                ...
                                <input
                                    ...
                                    required
                                />
                                <button type="submit" className="w-full bg-brand-700 text-white py-2 rounded-xl text-sm font-bold shadow-lg shadow-brand-100">
                                    {t('حفظ وإضافة الطالب')}
                                </button>
                            </form>
                        )}
            ...
            <Modal
                ...
                title={t('حساب ولي أمر')}
                subtitle={guardianFor?.name}
            >
                <form onSubmit={createGuardian} className="space-y-3">
                    <div className="bg-slate-50 rounded-2xl p-4 text-xs space-y-1">
                        <p className="font-black text-slate-700">
                            {t('اسم ولي الأمر')}: {guardianFor?.guardian_name || t('غير مسجل')}
                        </p>
                        <p className="font-bold text-slate-500" dir="ltr">
                            {guardianFor?.guardian_phone || '—'}
                        </p>
                    </div>

                    <p className="text-[11px] text-slate-500 font-bold leading-relaxed">
                        {t('يدخل ولي الأمر برقم هاتفه أعلاه، وكلمة المرور هي الرقم نفسه — وسيُطلب منه تغييرها بأول دخول.')}
                    </p>
                    <p className="text-[10px] text-slate-400 font-medium leading-relaxed">
                        {t('إذا كان لولي الأمر حساب بنفس رقم الهاتف، سيُضاف هذا الطالب إليه ولن تتغير كلمة مروره.')}
                    </p>
                    ...
                    >
                        {guardianBusy ? t('جاري الإنشاء...') : t('إنشاء أو ربط الحساب')}
                    </button>
                </form>
            </Modal>
            ...
            {viewingTeacher && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 font-sans">
                        <div className="p-6 bg-slate-50/50 border-b border-slate-50 flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                <div className="w-12 h-12 bg-brand-700 rounded-2xl flex items-center justify-center text-white font-bold text-lg">
                                    {viewingTeacher.name.charAt(0)}
                                </div>
                                <div>
                                    <h4 className="font-black text-slate-800 text-base">{viewingTeacher.name}</h4>
                                    <p className="text-[10px] font-bold text-brand-700 lowercase tracking-widest">{viewingTeacher.uid}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                <button
                                    onClick={() => setEditing({ user: viewingTeacher, kind: 'teacher' })}
                                    className="w-8 h-8 flex items-center justify-center bg-white rounded-xl shadow-sm border border-slate-100 text-slate-400 hover:text-brand-700"
                                    aria-label={t('تعديل بيانات الحساب')}
                                >
                        ...
                        <div className="p-6 space-y-6">
                                ...
                                <div className="space-y-2">
                                    {classes.filter(c => c.teacher_id === viewingTeacher.id || c.teacher_ids?.includes(viewingTeacher.id)).length > 0 ? (
                                        classes.filter(c => c.teacher_id === viewingTeacher.id || c.teacher_ids?.includes(viewingTeacher.id)).map(c => (
                                            <div key={c.id} className="p-3 bg-brand-50/50 rounded-xl border border-brand-100/50 flex justify-between items-center group/item">
                                                <div className="flex flex-col">
                                                    <span className="text-xs font-bold text-brand-900">{c.name}</span>
                                                    <span className="text-[8px] text-brand-400 font-bold">{t('مدرس الفصل')}</span>
                                                </div>
                                                {canManageTeachers && (
                                                    <button
                                                        onClick={(e) => {
                                                        ...
                                                        className="w-6 h-6 bg-white border border-brand-100 rounded-lg flex items-center justify-center text-red-400 hover:bg-red-50 hover:text-red-600 transition-all opacity-0 group-hover/item:opacity-100"
                            ...
                            {canManageTeachers && (
                                <div className="space-y-3 pt-2">
                                    ...
                                    <select
                                        className="w-full px-5 py-3 rounded-2xl border border-slate-100 bg-slate-50 outline-none text-xs font-bold focus:border-brand-400 transition-colors font-sans"
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`px-3 py-1.5 rounded-xl text-[10px] font-bold transition-all border ${isSelected
                                                    ? 'bg-brand-700 border-brand-700 text-white shadow-lg shadow-brand-100'
                                    ...
                                    <div key={idx} className="p-4 bg-white border border-slate-100 rounded-2xl flex items-center justify-between group hover:border-rose-200 transition-colors">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-0.5">
                                                    <span className="text-[9px] font-bold text-brand-700 bg-brand-50 px-2 py-0.5 rounded-lg">{abs.studentUid}</span>
