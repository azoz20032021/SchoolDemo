        ...
        <div className="p-4 md:p-6 space-y-4">
            <div className="flex flex-wrap items-start justify-between gap-3 print:hidden">
                <div className="min-w-0 flex-1">
                    <h2 className="text-xl font-black text-slate-900 tracking-tight">{t('التقارير')}</h2>
                    ...
                    {TABS.filter((tab_) => isStaff(user.role) || tab_.key !== 'finance').map((tab_) => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl text-xs font-black whitespace-nowrap transition-colors ${
                                tab === tab_.key ? 'bg-brand-700 text-white shadow-lg shadow-brand-100' : 'bg-white text-slate-500 border border-slate-100'
                            }`}
                        >
                            {t(tab_.label)}
                        </button>
            ...
            {user.role !== 'student' && (
                <Card className="p-4 space-y-3 print:hidden">
                    ...
                    <button
                        onClick={run}
                        disabled={loading}
                        className="w-full md:w-auto bg-brand-700 text-white px-6 py-2.5 rounded-xl text-xs font-black shadow-lg shadow-brand-100 disabled:opacity-60 flex items-center justify-center gap-1.5"
                    >
            ...
            {data && tab === 'student' && (
                <Sheet>
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-4 print-block">
                        {[
                            ['المعدل العام', data.grades.stats.overall_percentage === null ? '—' : `${data.grades.stats.overall_percentage}%`, 'text-brand-700'],
