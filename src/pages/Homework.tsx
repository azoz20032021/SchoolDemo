        ...
        <div className="p-4 flex items-start gap-3">
            <div
                className={`w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 ${
                    overdue ? 'bg-slate-100 text-slate-400' : 'bg-brand-50 text-brand-700'
                }`}
            >
...
export const Homework: React.FC<{ user: UserData }> = ({ user }) => {
    ...
    /**
     * A teacher is offered the subjects they were registered to teach — the
     * ones they chose when applying or the admin set on their account. Only
     * office staff, who set homework on anyone's behalf, see the full list.
     */
    const ownSubjects = user.subjects || [];
    const subjectOptions = user.role === 'teacher'
        ? (ownSubjects.length > 0 ? ownSubjects : subjects)
        : (subjects.length > 0 ? subjects : ownSubjects);

    return (
        <div className="p-4 md:p-6 space-y-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                    <h2 className="text-xl font-black text-slate-900 tracking-tight">{t('الواجبات')}</h2>
                ...
                {canAdd && classes.length > 0 && (
                    <button
                        ...
                        className="bg-gradient-to-l from-brand-700 to-brand-500 text-white px-4 py-2.5 rounded-xl text-xs font-black shadow-lg shadow-brand-700/30 active:scale-[0.98] transition-transform flex items-center gap-1.5 shrink-0"
                    >
