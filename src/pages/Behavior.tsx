...
const StudentView: React.FC<{ user: UserData }> = ({ user }) => {
    ...
    return (
        <div className="p-4 md:p-6 space-y-5">
            <div className="bg-brand-700 rounded-3xl p-6 text-white shadow-xl shadow-brand-100 relative overflow-hidden">
                <div className="relative z-10">
                    <p className="text-brand-100 text-[10px] uppercase tracking-widest font-bold mb-1">{t('درجة السلوك')}</p>
...
const NoteRow: React.FC<{ note: BehaviorNote; onDelete?: () => void; showStudent?: boolean }> = ({
    ...
    <div className="p-4 flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
            ...
            {showStudent && <p className="text-[11px] font-bold text-brand-600 mt-0.5">{note.student_name}</p>}
            {note.description && (
    ...
    return (
        <div className="p-4 md:p-6 space-y-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <h2 className="text-xl font-black text-slate-900 tracking-tight">{t('السلوك والملاحظات')}</h2>
                    <p className="text-xs text-slate-400 font-medium mt-0.5">{t('سجّل الملاحظات الإيجابية والسلبية لطلابك')}</p>
                </div>
                <button
                    onClick={() => setShowAdd(true)}
                    disabled={!selectedClass}
                    className="bg-gradient-to-l from-brand-700 to-brand-500 text-white px-4 py-2.5 rounded-xl text-xs font-black shadow-lg shadow-brand-700/30 active:scale-[0.98] transition-transform flex items-center gap-1.5 shrink-0 disabled:opacity-50"
                >
